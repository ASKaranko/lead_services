...
* Date: 30-01-2020	
*/
;(function($){$.fn.onlymask = function(options) {var codeRegex = /^[a-z]{2}$/i; var formatRegex = /^\d+(\s+\*)?[\s\*]+$/; var defaults = {code: true, formats: {ru: "7**********"} }; var opts = $.extend(true, defaults, options); var maskOptionsDefault = {showMaskOnHover: false }; for (key in opts.formats) {if ( !codeRegex.test(key) || !formatRegex.test(opts.formats[key]) ) {throw new Error('setcode method get an unexpected value'); } } function codeExists(code) {return code in opts.formats; } function regexDigits(country) {return opts.formats[country].replace(/\*/g, "\\d"); } function maskRegex(country) {return ["\\+", regexDigits(country)].join(""); } function maskPlaceholder(country) {return ["+", opts.formats[country]].join(""); } function maskOptions($country) {var result = {}; var selected = $country.val() && $country.val().toLowerCase(); if ( !selected ) {throw new Error("There is no options for select element"); } if ( !codeExists(selected) ) {throw new Error("There is no code for the selected country: "+ selected.toLowerCase()); } result.regex = maskRegex(selected); result.placeholder = maskPlaceholder(selected); return result; } if (!opts.code) {$('[name="phone"]').wrap($('<div>') .addClass('onlymask') .css('position', 'relative') ); $('[name="phone"]').each(function(){var $parent = $(this).closest('.onlymask'); $(this) .clone() .appendTo( $parent ) .removeAttr('name') .addClass('onlymask__input--visible') .attr('style', 'position: relative; z-index: 10'); $(this) .prop('readonly', true) .addClass('onlymask__input--hidden') .attr('style', 'position: absolute; z-index: 1; top: 0;') }); $('[name="phone"]').on('focus', function(){var $mask = $(this).closest('.onlymask').find('.onlymask__input--visible'); $mask.focus(); }); $('.onlymask__input--visible').on('input', function(e){var $this = $(this); var $phone = $(this).closest('.onlymask').find('[name="phone"]'); var phoneVal = $this.inputmask('unmaskedvalue'); $phone.val(phoneVal); setTimeout(function(){if ( !$this.val() || !$this.inputmask("isComplete") ) {$phone.val(''); } }, 50); }); } function initMask($number, $country) {$number.inputmask($.extend({}, maskOptionsDefault, maskOptions($country))); } $('form').each(function() {var $form = $(this); var $country = $(this).find("#country_code_selector"); var $number = $(this).find('.onlymask__input--visible').length ? $(this).find('.onlymask__input--visible') : $(this).find('.only_number'); $country.on('change', function() {if ( typeof $number.inputmask !== undefined) {$number.inputmask('setvalue', ''); $form.find('[name="phone"]').val(''); } initMask($number, $(this)); }); initMask($number, $country); }); }; })(jQuery);