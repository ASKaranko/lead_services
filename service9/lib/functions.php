...
function form_big($order){
    ...
    if ($id = order_get_id()) {
        ...
        if (!isset($order['price']) && !isset($current_order['price'])) {
            $order['price'] = get_price();
        }

        if (!isset($order['currency']) && !isset($current_order['currency'])) {
            $order['currency'] = get_currency();
        }

        $order = array_merge($current_order, $order);


        if (isset($order['quantity'])) {
            $order['price'] =  (isset($order['base_price']) ? $order['base_price'] : $order['price'])  * $order['quantity'];
        } elseif (isset($current_order['quantity'])) {
            $order['price'] =  (isset($order['base_price']) ? $order['base_price'] : $order['price'])  * $current_order['quantity'];
        }
...
function get_res($file)
{
    ...
    if(isset($da_config['iframe']) && $da_config['iframe']) {
        return '/examples/php/simple_standard_extended_18/sites/'.$file;
...
function get_price_extended2()
{
    $prices = get_product_prices();
    return $prices['price_extended2'];
